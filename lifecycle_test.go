...
func TestLifecycleReconnectsPromptlyAfterTCPReset(t *testing.T) {
	...
	lc := NewLifecycle(LifecycleConfig{InitialBackoff: 3 * time.Second, MaxBackoff: 3 * time.Second, FlapWindow: -1}, c)
...
func TestLifecyclePingWatchdogReconnectsAndRecovers(t *testing.T) {
	...
	lc := NewLifecycle(LifecycleConfig{InitialBackoff: 2 * time.Second, MaxBackoff: 2 * time.Second, FlapWindow: -1}, c)
